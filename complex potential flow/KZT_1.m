...
Cp(indx) = 0.0;
figure(1)
%flow around circle in z-plane
contour(x,y,streamfunction,levels)
axis equal
axis([xmin xmax ymin ymax])
title('flow in z-plane')
%draw circle
hold on
plot(x_c,y_c,'k')
fill(x_c,y_c,'y')
plot(xTE,0,'or',xLE,0,'og')
hold off

figure(2)
%flow around airfoil in ksi plane
contour(zeta,eta,streamfunction,levels)
axis equal
axis([xmin xmax ymin ymax])
title('flow in Ksi-plane')
%draw Airfoil
hold on
plot(ksi_a,'k')
fill(zeta_a,eta_a,'y')
...
figure(4)
levels = linspace(-10,1,100);
contourf(zeta,eta,Cp,levels)
colormap('spring')
axis equal
axis ([xmin xmax ymin ymax])
hold on 
